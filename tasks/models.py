...
class DeepFaceModel(models.Model):
    ...
    model_key = models.CharField(u'appkey', max_length=200)
    model_headers = models.CharField(u'headers', max_length=200)
    created_time = models.DateTimeField(u'创建时间', auto_now_add=True, null=True)

    class Meta:
        db_table = 'deepface_model'
        verbose_name = u'模型管理'
...
class TaskInfo(models.Model):
    ...
    def format_time(self):
        ...
        if timedelta.find('minute') != -1: 
            ...
            if minutes <> '0':
                return minutes + u'分钟前'
            else:
                return u'刚刚'
        elif timedelta.find('hour') != -1: 
            return timedelta.split('\xa0')[0] + u'小时前'
        elif timedelta.find('day') != -1: 
            return timedelta.split('\xa0')[0] + u'天前'
        elif timedelta.find('month') != -1: 
            return timedelta.split('\xa0')[0] + u'个月前'
        elif timedelta.find('week') != -1: 
            return timedelta.split('\xa0')[0] + u'周前'
        elif timedelta.find('year') != -1: 
            return timedelta.split('\xa0')[0] + u'年前'
        else:
            return timedelta
...
class FaceTrack(models.Model):
    ...
    isnewlycreated = models.IntegerField(u'创建or合并', default=0)
...
class Person(models.Model):
    ...
    model = models.ForeignKey(DeepFaceModel, verbose_name=u'任务模型', on_delete=models.CASCADE)
